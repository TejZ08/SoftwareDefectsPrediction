...
nn_model.add(tf.keras.layers.Dense(50,activation='relu',input_shape=(X_train.shape[1],)))
nn_model.add(tf.keras.layers.Dropout(0.2))
nn_model.add(tf.keras.layers.Dense(25,activation='relu'))
nn_model.add(tf.keras.layers.Dropout(0.2))
nn_model.add(tf.keras.layers.Dense(1,activation='sigmoid'))
nn_model.compile(optimizer=tf.keras.optimizers.legacy.RMSprop(learning_rate=0.01,decay=0.0000001),loss='binary_crossentropy',metrics=[tf.keras.metrics.BinaryAccuracy()])
...
fig = plt.figure()
plt.bar(['train','test'],[nb_score_train,nb_score_test])
plt.ylabel('accuracy(%)')
plt.savefig('graph_nb.png')
plt.clf()

fig = plt.figure()
plt.bar(['Naive Bayes','Logistic Regression','Neural Netwrk'],[nb_score_test,lr_score_test,nn_score_test])
plt.xlabel('model')
plt.ylabel('accuracy(%)')
...
fig = plt.figure()
plt.bar(['Naive Bayes','Logistic Regression','Neural Netwrk'],[nb_f1_test,lr_f1_test,nn_f1_test])
plt.xlabel('model')
plt.ylabel('accuracy(%)')
plt.savefig('compare_f1.png')
plt.clf()
